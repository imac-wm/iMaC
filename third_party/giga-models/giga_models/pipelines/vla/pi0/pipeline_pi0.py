...
class Pi0Pipeline(BasePipeline):
    ...
    def __init__(
        ...
        gripper_zero_threshold: float | None = None,
    ) -> None:
        ...
        Args:
            ...
            adapt_to_pi: Whether to adapt Aloha inputs/outputs to the pi0 runtime space.
            gripper_zero_threshold: If set, zero action dims 6 and 13 when their values are below this threshold.
        """
        super().__init__()
        ...
        # Input transforms
        self.aloha_inputs_transform = AlohaInputs(adapt_to_pi=adapt_to_pi)
        ...
        self.aloha_outputs_transform = AlohaOutputs(original_action_dim=original_action_dim, adapt_to_pi=adapt_to_pi)

    def _zero_small_gripper_actions(self, action: torch.Tensor) -> torch.Tensor:
        if self.gripper_zero_threshold is None:
            return action

        gripper_indices = [idx for idx in (6, 13) if idx < action.shape[-1]]
        if not gripper_indices:
            return action

        action = action.clone()
        gripper_actions = action[..., gripper_indices]
        action[..., gripper_indices] = torch.where(
            gripper_actions < self.gripper_zero_threshold,
            torch.zeros_like(gripper_actions),
            gripper_actions,
        )
        return action
    ...
    def __call__(
        ...
        # Output transforms
        output_dict = {'action': pred_action[0][..., : self.original_action_dim], 'observation.state': normalized_state}
        ...
        pred_action = self._zero_small_gripper_actions(pred_action).to(ori_device)
