...
from ..models.transformer_worldarena_3d import WanTransformer3DModelWorldArena3D
...
class WorldArena3DTrainer(Trainer):
    def get_models(self, model_config):
        ...
        self.use_replay_condition = model_config.get("use_replay_condition", True)
        self.use_scene_3d_condition = model_config.get("use_scene_3d_condition", True)
        self.use_replay_3d_condition = model_config.get("use_replay_3d_condition", True)
        self.scene_3d_weight = float(model_config.get("scene_3d_weight", 1.0))
        self.replay_3d_weight = float(model_config.get("replay_3d_weight", 1.0))
        model = dict()
        # vae
        ...
        if model_config.get("unpretrain", False):
            ...
            transformer = WanTransformer3DModelWorldArena3D.from_config(transformer_pretrained, torch_dtype=self.dtype)
        else:
            transformer = WanTransformer3DModelWorldArena3D.from_pretrained(transformer_pretrained, torch_dtype=self.dtype)
            transformer.replay_embedding = copy.deepcopy(transformer.patch_embedding)
            transformer.scene_3d_embedding = copy.deepcopy(transformer.patch_embedding)
            transformer.replay_3d_embedding = copy.deepcopy(transformer.patch_embedding)
    ...
    def prepare_conditioning(self, batch_dict):
        ...
        replay = batch_dict.get('replay_condition_images', None)
        scene_3d = batch_dict.get('scene_3d_condition_images', None)
        replay_3d = batch_dict.get('replay_3d_condition_images', None)
        # inputs
        front_latents = self.forward_vae(front_images)
        replay_latents = self.forward_vae(replay) if (self.use_replay_condition and replay is not None) else None
        scene_3d_latents = self.forward_vae(scene_3d) if (self.use_scene_3d_condition and scene_3d is not None) else None
        replay_3d_latents = self.forward_vae(replay_3d) if (self.use_replay_3d_condition and replay_3d is not None) else None
        ...
        condition['scene_3d_latents'] = scene_3d_latents
        condition['replay_3d_latents'] = replay_3d_latents
        condition['prompt_embeds'] = prompt_embeds.to(dtype=latents.dtype, device=latents.device)
        # Keep condition injection scale stable when extra 3D branches are enabled.
        # This preserves the pre-weight-change behavior and avoids over-amplifying
        # transformer inputs when both scene_3d and replay_3d are active.
        active_extra_weight = 0.0
        if scene_3d_latents is not None:
            active_extra_weight += max(self.scene_3d_weight, 0.0)
        if replay_3d_latents is not None:
            active_extra_weight += max(self.replay_3d_weight, 0.0)
        condition_scale = 1.0 / (1.0 + active_extra_weight)
        condition['scene_3d_weight'] = self.scene_3d_weight * condition_scale
        condition['replay_3d_weight'] = self.replay_3d_weight * condition_scale
        return condition
    ...
    def denoise_net(self, transformer, xt, sigma, condition, add_ref_aug=False, return_x0=False):
        ...
        scene_3d_latents = condition['scene_3d_latents']
        replay_3d_latents = condition['replay_3d_latents']
        ...
        scene_3d_weight = condition.get('scene_3d_weight', 1.0)
        replay_3d_weight = condition.get('replay_3d_weight', 1.0)
        ...
        timestep = (first_frame_mask[:, :, :, ::2, ::2] * t).reshape(bs, -1)
    ...
    def rollout(self, batch_dict, id):
        ...
        scene_3d_latents = condition['scene_3d_latents']
        replay_3d_latents = condition['replay_3d_latents']
        should_visualize = self.process_index == 0 and (
            (self.cur_step % self.view_interval == 0) or (self.cur_step == 1 and len(self._outputs) == 0)
        )
        if should_visualize:
            self.vae_decode(ref_latents, sign=f"ref_{id}")
            self.vae_decode(latents, sign=f'input_rollout_step_{id}')
            if replay_latents is not None:
                self.vae_decode(replay_latents, sign=f'replay_condition_step_{id}')
            if scene_3d_latents is not None:
                self.vae_decode(scene_3d_latents, sign=f'scene_3d_condition_step_{id}')
            if replay_3d_latents is not None:
                self.vae_decode(replay_3d_latents, sign=f'replay_3d_condition_step_{id}')
    ...
    def forward_step(self, batch_dict):
        ...
        replay_3d = batch_dict.get('replay_3d_condition_images', None)
        prompt_embeds = batch_dict['prompt_embeds']
        if self.use_replay_condition and replay is not None:
            assert replay.shape == front_images.shape
        if self.use_scene_3d_condition and scene_3d is not None:
            assert scene_3d.shape == front_images.shape
        if self.use_replay_3d_condition and replay_3d is not None:
            assert replay_3d.shape == front_images.shape
        num_ref_images = 1
        loss_dict = {}
        ...
        for i in range(self.rollout_step):
            ...
            if self.use_replay_condition and replay is not None:
                roll_dict['replay_condition_images'] = replay[:, start_frame:end_frame]
            if self.use_scene_3d_condition and scene_3d is not None:
                roll_dict['scene_3d_condition_images'] = scene_3d[:, start_frame:end_frame]
            if self.use_replay_3d_condition and replay_3d is not None:
                roll_dict['replay_3d_condition_images'] = replay_3d[:, start_frame:end_frame]
        ...
        with torch.no_grad():
            ...
            if should_visualize:
                ...
                if latents is not None:
                    ...
                    with torch.no_grad():
                        tensor_video = self.vae.decode(latents[:1], return_dict=False)[0].detach()
