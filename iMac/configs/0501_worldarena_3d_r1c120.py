...
dst_size = (320, 256)
num_frames = 120 # SeedVR-2 -> 1280 or higher
rollout = 1
total_frames = num_frames * rollout + 1
fps = 16
project_dir = os.environ.get(
    "WORLDARENA_OUTPUT_DIR",
    os.path.join(OUTPUT_DIR, "worldarena_3d"),
)
config = dict(
    project_dir=project_dir,
    runners=["iMac.trainer.WorldArena3DTrainer"],
    launch=dict(
        gpu_ids=[0, 1, 2, 3],
        # gpu_ids=[0],
    ...
    dataloaders=dict(
        train=dict(
            data_or_config=[
                os.path.join(WORLDARENA_DATA_DIR, "train"),
                os.path.join(WORLDARENA_DATA_DIR, "val"),
            ],
            ...
            filter=dict(
                mode='overall_func',
                func='iMac.configs.0501_worldarena_3d_r1c120.filter_data',
                dst_size=dst_size,
            ...
            transform=dict(
                ...
                is_train=True,
                fps=fps,
                default_prompt_embeds_path=DEFAULT_PROMPT_EMBEDDING_PATH,
...
def filter_data(
    ...
    # keys = list(video_info_dict.keys())
    # keys.sort()
    # total_size = 0
    # for key in keys:
    #     val = video_info_dict[key]
    #     if len(val) >= min_size:
    #         total_size += len(val)
    #         print(key, len(val), total_size, key[0] * key[1])
    # print(total_size)
    # exit(0)
