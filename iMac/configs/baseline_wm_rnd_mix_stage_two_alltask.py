...
project_dir = os.environ.get(
    ...
    os.path.join(OUTPUT_DIR, "rnd_mix_stage_two_alltask"),
)
stage_one_checkpoint = os.environ.get("RND_MIX_STAGE_ONE_CHECKPOINT")
if not stage_one_checkpoint:
    raise ValueError(
        "RND_MIX_STAGE_ONE_CHECKPOINT must point to a stage-one transformer checkpoint."
    )

config = dict(
    project_dir=project_dir,
    runners=["iMac.trainer.BaselineWMRNDMixStageTwoTrainer"],
    launch=dict(
    ...
    dataloaders=dict(
        train=dict(
            ...
            filter=dict(
                mode='overall_func',
                func='iMac.configs.baseline_wm_rnd_mix_stage_two_alltask.filter_data',
                dst_size=dst_size,
            ...
            transform=dict(
                type='WMRNDMixStageTwoTransforms',
                dst_size=dst_size,
    ...
    models=dict(
        ...
        generator=dict(
            ...
            gripper_mesh_dir=os.environ.get("PIPER_GRIPPER_MESH_DIR"),
            load_da3_model=False,
            depth_point2double_cond=dict(
                use_gpu=False,
                gpu_dist_chunk_size=1024,
            ),
        ),
    ),
