...
class Action_encoder(nn.Module):
    def __init__(self, action_dim = 14, hidden_size = 4096, text_cond=True):
        super().__init__()
    ...
    def forward(self, action,  texts=None, text_tokinizer=None, text_encoder=None):
...
class WanTransformer3DModel(
    ...
    def __init__(
        ...
        self.encoder = Action_encoder()

    def load_action_encoder(self, encoder_model_path):
        # load action encoder weights from checkpoint
        state_dict = torch.load(encoder_model_path, map_location="cpu")
        self.encoder.load_state_dict(state_dict)
        logger.info(f"Loaded action encoder from {encoder_model_path}")
...
class WanTransformer3DModel_action(
    ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin, CacheMixin, AttentionMixin
):
    r"""
    A Transformer model for video-like data used in the Wan model.
    ...
    def __init__(
        ...
        self.rope = WanRotaryPosEmbed(attention_head_dim, patch_size, rope_max_seq_len)
        self.patch_embedding = nn.Conv3d(in_channels, inner_dim, kernel_size=patch_size, stride=patch_size)
        self.action_embedding = Action_encoder(action_dim=7)
    ...
    def forward(
        ...
        if action is not None:
            action_states = self.action_embedding(action)
            length = action_states.shape[-2]
            encoder_hidden_states[:, :length] = action_states
...
class WanTransformer3DModel_mixed(
    ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin, CacheMixin, AttentionMixin
):
    r"""
    A Transformer model for video-like data used in the Wan model.
    ...
    def __init__(
        ...
        self.condition_embedder = WanTimeTextImageEmbedding(
            dim=inner_dim,
            time_freq_dim=freq_dim, # B C(48) T C/16 W # (B N D)
            time_proj_dim=inner_dim * 6,
            text_embed_dim=text_dim,
            image_embed_dim=image_dim,
            pos_embed_seq_len=pos_embed_seq_len,
...
class WanTransformer3DModel_depth(
    ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin, CacheMixin, AttentionMixin
):
    r"""
    A Transformer model for video-like data used in the Wan model.
    ...
    def forward(
        ...
        if depth_latents is not None:
            depth_states = self.depth_embedding(depth_latents)
            hidden_states = hidden_states + depth_states
